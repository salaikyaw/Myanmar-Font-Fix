...
if (!Number.isInteger(port) || port < 19431 || port > 19599 || !/^[a-z]+$/.test(app)) throw Error('Invalid target');
const css = fs.readFileSync(path.join(__dirname, 'font.css'), 'utf8');
const appConfig=JSON.parse(fs.readFileSync(path.join(__dirname,'apps.json'),'utf8')).find(a=>a.id===app);
